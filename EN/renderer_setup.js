(async () => {
    $(document).ready(function() {
        var t = $('#settingstable').DataTable({
           language: {
          emptyTable: "There is no meeting set up"
        },
          responsive: true,
        searching: false, 
          paging: false, 
     ...
     async function addall () {
        ...
        t.row.add( [
            `<input type="text" value="${element.name}" placeholder="Class Name" required/>`,
            `<input type="text" value="${element.code}" placeholder="Class Code" required/>`,
            `<input type="color" value="${element.color}" required/>`,
          '<button class="remove" value="false" style="background-color:#b35050;">DELETE</button>'
        ] ).draw( false );
    });
    };
