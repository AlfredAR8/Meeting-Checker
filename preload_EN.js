...
const spawnservicechild = async () => {
  if (AppConfig[0].DiscordIntegration == "true") {
    const jsfilename = path.resolve(__dirname, './EN/MeetingCheckerServiceDiscord.js')
    var child = fork(jsfilename, [`${appdatalocation}`], { silent: false });
  } else {
    const jsfilename = path.resolve(__dirname, './EN/MeetingCheckerService.js')
    var child = fork(jsfilename, [`${appdatalocation}`], { silent: false });
  }
  ...
  const spawnservicechildreload = async () => {
    const jsfilename = path.resolve(__dirname, './EN/reloadaccount.js')
...
const saveclasslist = async (jsonfile) => {
  ...
  [System.Windows.MessageBox]::Show('Your changes has been saved! 
(Restart the app for see the changes)','Saved!','OK','Asterisk');
  `]);
}
