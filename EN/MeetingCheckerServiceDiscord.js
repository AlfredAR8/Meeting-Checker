...
client.on('ready', () => {
  ...
  [System.Windows.MessageBox]::Show('The Discord Bot Token is Wrong or you dont have internet connection, please verify it or disable the Discord DM option in settings.
After that stop and start the bot again','Bot Token','OK','Error');
  `]);
  process.exit()
}
...
.setTitle(`The ${element.name} meeting has been enabled!`)
.setAuthor(`Meeting Checker`, 'https://raw.githubusercontent.com/AlfredAR8/MeetingCheckerAPI/main/AppIcon.png')
.setDescription(`Link: https://meet.google.com/lookup/${element.code}`)
await client.users.fetch(AppConfig[0].DiscordUserID).then((user) => {
  ...
  spawnSync("powershell.exe", [`
  Add-Type -AssemblyName PresentationCore,PresentationFramework;
  [System.Windows.MessageBox]::Show('The Discord User ID is Wrong, you dont sare server with the bot or you dont have internet connection, please verify your id, add the bot to a server you created or disable the Discord DM option in settings.
After that stop and start the bot again','Discord User ID','OK','Error');
  `]);
  process.exit()
}
var VersionCheck1 = fs.readFileSync(`${locationprocess}/UserConfig/Cache/MeetingsTimeline.json`)
